...
class DialogManager {
  ...
  getDialog(variant?: "danger" | "primary" | "success"): HTMLDialogElement {
    ...
    let className =
      "rounded-2xl bg-white/70 dark:bg-zinc-900/70 backdrop-blur-xl border border-white/20 dark:border-zinc-700/30 shadow-xl fixed left-1/2 top-1/2 w-full max-w-md -translate-x-1/2 -translate-y-1/2 p-4 backdrop:bg-black/30 backdrop:backdrop-blur-sm";
      ...
      (close) => (
        <div>
          ...
          <div class="flex justify-end gap-3">
            <button
              onClick={() => close()}
              class={`${options?.variant === "danger" ? "btn-danger" : options?.variant === "success" ? "btn-success" : "btn-primary"} px-4 py-2`}
            >
